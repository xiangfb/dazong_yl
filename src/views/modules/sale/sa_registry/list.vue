...
export default {
  ...
  methods: {
    popupConfirm () { // 弹窗资源点击确定触发  data为已选跨页数据
      ...
      let noticeId = getList[0].mainId
      SaRegistryService.updateWeighsByNotice({
        noticeId: noticeId,
    ...
    getMaterialList () {
      ...
      SaRegistryService.list(params).then(res => {
        if (res.success) {
    ...
    supplementCard (code, id) {
      SaRegistryService.supplementCard({
        id: id,
        cardCode: code
      }).then(res => {
        if (res.success) {
      ...
      switch (e) {
        ...
        case 'updateNotice':
          ...
          for (var a = 0; a < getList.length; a++) {
            if (!(getList[a]['sourceMainCode'] === '' || getList[a]['sourceMainCode'] == null)) {
              this.$message.error('选中项不是本地单据，不允许修改订单')
              return
            }
          }
          this.$refs.com.$refs.custom.$refs.editable.getSelecteds().map(item => {
          ...
          GlobalService.optimusPrimePopup('345').then(res => {
            if (res.success) {
        ...
        case 'copy':
          ...
          let id = getList[0].id
          SaRegistryService.copy(id).then(res => {
            if (res.success) {
              this.$message.success('操作成功')
              this.getMaterialList()
            }
        ...
        case 'againMatch':
          ...
          SaRegistryService.againMatch(getList[0].id).then(res => {
            if (res.success) {
              this.$message.success('操作成功')
              this.getMaterialList()
            }
        ...
        case 'upControlPoint':
          ...
          console.log(getList[0])
          SaRegistryService.upControlPoint({
            id: getList[0].id,
        ...
        case 'downControlPoint':
          ...
          console.log(getList[0])
          SaRegistryService.downControlPoint({
            id: getList[0].id,
        ...
        case 'picture':
          ...
          console.log(getList[0])
          SaRegistryService.picture(getList[0].id).then(res => {
            this.$message.success(res.data)
          })
          break
        case 'safeUpdate':
          ...
          currentId = getList[0].id
          SaRegistryService.getSub(getList[0].id).then(res => {
            if (!res.data[0] || res.data[0].deWeigh01 === null || res.data[0].deWeigh01 === '') {
              ...
              setTimeout(function t () {
                if (k < serviceidWrite.length) {
                  if (id !== undefined) {
                    // 开始写入值
                    block.push('4')
                    dataes.push('0000000000000002') // 卡型  2-采样
                    block.push('5')
                    dataes.push('sdxwwcsv20071210') // 系统标示
                    block.push('6')
                    dataes.push(main.plateNumber === null ? '' : main.plateNumber) // 车号
                    block.push('8')
                    dataes.push('') // 入场登记单号
                    block.push('9')
                    ...
                    dataes.push('') // 车型
                    block.push('28')
                    dataes.push('1' + obj.strReplace(child[0].preNums) + obj.strReplace(child[0].preQuantity) + '0') // 行号+预装件数+预装吨数+装货标志
                    block.push('29')

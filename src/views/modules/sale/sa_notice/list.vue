...
import SaNoticeService from 'service/modules/sale/sa_notice/sa-notice-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getMaterialList () {
      ...
      SaNoticeService.list(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        ...
        case 'copy': // 复制一条信息按钮（字段名称与菜单中配置的名称对应）
          ...
          let id = getList[0].id
          SaNoticeService.copy(id).then(res => {
            this.$message.success(res.msg)
          })
          break
      }

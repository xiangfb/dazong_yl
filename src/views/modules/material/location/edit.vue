...
import LocationService from 'service/modules/material/location/location-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      ...
      LocationService.formData(params).then(res => {
        if (res.success) {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_FROM') {
        LocationService.trees().then(tree => {
          if (tree.success) {
    ...
    treeHandle (data) {
      data.forEach(ele => {
        ele.name = ele.jsonObject.locationName
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        LocationService.add(this.param.parentId).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'cadd') {
        LocationService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'delete') {
        LocationService.delete(this.param.id).then(res => {
          if (res.success) {

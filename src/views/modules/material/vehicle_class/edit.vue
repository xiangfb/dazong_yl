...
import VehicleClassService from 'service/modules/material/vehicle_class/vehicle-class-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    getMould () { // 获取当前菜单配置
      VehicleClassService.optimusPrime({
        formatType: 'FORM',
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            VehicleClassService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/vehicle_class/list')
          break
      }
    }
  },

...
import CustomerClassService from 'service/modules/material/customer_class/customer-class-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      ...
      CustomerClassService.formData(params).then(res => {
        if (res.success) {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_FROM') {
        CustomerClassService.trees().then(tree => {
          if (tree.success) {
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        CustomerClassService.add(this.param.parentId).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'cadd') {
        CustomerClassService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'synchronization') {
        CustomerClassService.syn({
        ...
        CustomerClassService.delete(this.param.id).then(res => {
          if (res.success) {

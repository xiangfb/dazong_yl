  ...
  methods: {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_LIST') {
        ComputationService.trees().then(tree => {
          if (tree.success) {
    ...
    treeHandle (data) {
      data.forEach(ele => {
        ele.name = ele.jsonObject.comgName
    ...
    getData (id) {
      ...
      if (this.searchId) {
        params.search = [{ key: 'comgId', op: 'eq', val: this.searchId }]
      }
      ComputationService.formData(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        case 'delete':
          ...
          ComputationService.delete(result).then(res => {

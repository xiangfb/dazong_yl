...
import BatchService from 'service/modules/material/batch/batch-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
      ...
      OptimusPrimeService.optimusPrime({
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            BatchService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/batch/list')
          break
      }
    }
  },

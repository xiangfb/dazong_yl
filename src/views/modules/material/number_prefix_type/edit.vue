...
export default {
  ...
  methods: {
      ...
      NumberPrefixTypeService.optimusPrime({
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            NumberPrefixTypeService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    submitForm (state, modelData) {
      ...
      if (state) { // 提交成功.
        if (this.$route.query.detailId) {
          NumberPrefixTypeService.update({
            entity: modelData
          }).then(res => {
            if (res.success) {
              this.$message({
          ...
          NumberPrefixTypeService.create({
            entity: modelData
          }).then(res => {
            if (res.success) {
              this.$message({
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/number_prefix_type/list')
          break
      }
    }
  },

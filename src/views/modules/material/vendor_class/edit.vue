...
import VendorClassService from 'service/modules/material/vendor_class/vendor-class-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      ...
      VendorClassService.formData(params).then(res => {
        if (res.success) {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_FROM') {
        VendorClassService.trees().then(tree => {
          if (tree.success) {
    ...
    treeHandle (data) {
      data.forEach(ele => {
        ele.name = ele.jsonObject.vencName
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        VendorClassService.add(this.param.parentId).then(res => {
          if (res.success) {
        ...
        console.log(this.param.parentId)
        VendorClassService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'synchronization') {
        VendorClassService.syn({
        ...
        VendorClassService.delete(this.param.id).then(res => {
          if (res.success) {
            console.log(res.data)

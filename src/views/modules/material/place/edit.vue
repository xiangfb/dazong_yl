<template>
  <div :style="{height: `${maxHeight}px`}" v-loading="loading">
    <component :is="currentView" :tips="tips" :btnGroup="btnGroup" :max-height="maxHeight" :fieldData="fieldData" :modelData="modelData" @but-receipt="butEvent" @submit="submitForm" @reset="resetForm"></component>
  </div>
</template>
...
import PlaceService from 'service/modules/material/place/place-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
      ...
      PlaceService.optimusPrime({
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            PlaceService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    butEvent (type) {
      ...
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/place/list')
          break
      }
    }
  },

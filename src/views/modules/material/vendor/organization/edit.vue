...
import VendorOrganizationService from 'service/modules/material/vendor/organization/vendor-organization-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
      ...
      OptimusPrimeService.optimusPrime({
        ...
        if (res.success) {
          ...
          res.data.formSubList.forEach((item, index) => {
            ...
            this.modelData['as_vendor_ex_allocate'][item.fieldName] = item.defaultValue
          })
    ...
    getInfo (id) {
      VendorOrganizationService.detail(id).then(res => {
        if (res.success) {
          this.modelData = {
            'as_vendor': res.data.entity, // 要与tabOptions的name一一对应
            'as_vendor_ex_allocate': res.data.entitysub
          }
        }
      })
    },
    submitForm (state, modelData) {
      ...
      if (state) { // 提交成功.
        switch (this.$route.query.type) {
          case 'create':
            VendorOrganizationService.save({
              entity: modelData.as_vendor,
              entitysub: modelData.as_vendor_ex_allocate
            }).then(res => {
              console.log(res)
              if (res.success) {
            ...
            break
          case 'update':
            VendorOrganizationService.update({
              entity: modelData.as_vendor,
              entitysub: modelData.as_vendor_ex_allocate
            }).then(res => {
              console.log(res)
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/vendor/organization/list')
          break
      }
    }
  },

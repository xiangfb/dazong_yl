...
import OrganizationService from 'service/modules/material/organization/organization-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_FROM') {
        OrganizationService.trees().then(tree => {
          if (tree.success) {
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        OrganizationService.add(this.param.parentId).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'cadd') {
        OrganizationService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
        ...
        OrganizationService.delete(this.param.id).then(res => {
          if (res.success) {

  ...
  methods: {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_LIST') {
        InventoryGroupService.trees().then(tree => {
          if (tree.success) {
            this.treeData = this.treeHandle(tree.data)
            this.searchId = this.treeData && this.treeData[0] && this.treeData[0].id
            this.getData()
          } else {
            this.getData()
          }
    ...
    treeHandle (data) {
      data.forEach(ele => {
        ele.name = ele.jsonObject.invcName
    ...
    getData () {
      ...
      if (this.searchId) {
        params.search = [{ key: 'invcId', op: 'eq', val: this.searchId }]
      }
      ...
      InventoryGroupService.formData(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        case 'delete':
          ...
          InventoryGroupService.delete(result).then(res => {
        ...
        case 'distribute':
          ...
          InventoryGroupService.organizationTrees().then(res => {
            if (res.success) {
        ...
        case 'cancelAllDistribute':
          ...
          InventoryGroupService.cancelAll({
            entity: {
              'archivesIds': result
            }
          }).then(res => {
        ...
        case 'distributeSearch':
          ...
          InventoryGroupService.findAllocate(getList[0].id).then(res => {
            if (res.success) {
              console.log(res)
    ...
    popupSubmit () { // 按钮点击触发的弹窗需要
      ...
      console.log(orgList)
      if (this.flag === 'fp') {
        InventoryGroupService.allocate({
          entity: {
            archivesIds: venList,
            orgIds: orgList
          }
      ...
      if (this.flag === 'cfp') {
        InventoryGroupService.cancelAllocate({
          entity: {
            archivesIds: venList,
            orgIds: orgList
          }

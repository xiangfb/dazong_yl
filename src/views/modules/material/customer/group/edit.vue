...
export default {
  ...
  methods: {
      ...
      OptimusPrimeService.optimusPrime({
        ...
        if (res.success) {
          ...
          res.data.formSubList.forEach((item, index) => {
            ...
            this.modelData['as_customer_ex'][item.fieldName] = item.defaultValue
          })
    ...
    getInfo (id) {
      CustomerGroupService.detail(id).then(res => {
        if (res.success) {
          this.modelData = {
            'as_customer': res.data.entity, // 要与tabOptions的name一一对应
            'as_customer_ex': res.data.entitysub
          }
        }
      })
    },
    submitForm (state, modelData) {
      ...
      if (state) { // 提交成功.
        switch (this.$route.query.type) {
          case 'create':
            CustomerGroupService.save({
              entity: modelData.as_customer,
              entitysub: modelData.as_customer_ex
            }).then(res => {
              console.log(res)
              if (res.success) {
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/customer/group/list')
          break
      }
    }
  },

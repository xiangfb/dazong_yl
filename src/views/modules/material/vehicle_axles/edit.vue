...
import VehicleAxlesService from 'service/modules/material/vehicle_axles/vehicle-axles-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    getMould () { // 获取当前菜单配置
      VehicleAxlesService.optimusPrime({
        formatType: 'FORM',
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            VehicleAxlesService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/vehicle_axles/list')
          break
      }
    }
  },

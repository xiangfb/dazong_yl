...
import ComputationGroupService from 'service/modules/material/computation_group/computation-group-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    getMould () { // 获取当前菜单配置
      ComputationGroupService.optimusPrime({
        formatType: 'FORM',
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            ComputationGroupService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    submitForm (state, modelData) {
      ...
      if (state) { // 提交成功.
        if (this.$route.query.detailId) {
          ComputationGroupService.update({
            entity: modelData
          }).then(res => {
            if (res.success) {
              this.$message({
          ...
          ComputationGroupService.create({
            entity: modelData
          }).then(res => {
            if (res.success) {
              this.$message({
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/computation_group/list')
          break
      }
    }
  },

...
import SysBusiTypeService from 'service/modules/material/sys_busi_type/sys-busi-type-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    getMould () { // 获取当前菜单配置
      SysBusiTypeService.optimusPrime({
        formatType: 'FORM',
        ...
        if (res.success) {
          ...
          if (this.$route.query.detailId) {
            SysBusiTypeService.detail(this.$route.query.detailId).then(res => {
              if (res.success) {
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/sys_busi_type/list')
          break
      }
    }
  },

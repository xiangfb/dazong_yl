...
import InventoryClassService from 'service/modules/material/inventory_class/inventory-class-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      ...
      InventoryClassService.formData(params).then(res => {
        if (res.success) {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_FROM') {
        InventoryClassService.trees().then(tree => {
          if (tree.success) {
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        InventoryClassService.add(this.param.parentId).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'cadd') {
        console.log(this.param.parentId)
        InventoryClassService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'synchronization') {
        InventoryClassService.syn({
        ...
        InventoryClassService.delete(this.param.id).then(res => {
          if (res.success) {

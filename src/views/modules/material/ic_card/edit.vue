...
export default {
  ...
  methods: {
      ...
      IcCardService.optimusPrime({
        ...
        if (res.success) {
          ...
          if (this.$route.query.detail) {
            // this.getInfo(this.$route.query.detail)
            this.modelData = this.$route.query.detail
          }
    ...
    submitForm (state, modelData) {
      ...
      if (state) { // 提交成功.
        if (this.$route.query.detail) {
          IcCardService.update({
            entity: modelData
          }).then(res => {
            console.log(res)
          })
        } else {
          IcCardService.create({
            entity: modelData
          }).then(res => {
          // this.$message({
          //   message: '提交成功！',
    ...
    butEvent (type) {
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/ic_card/list')
          break
      }
    }
  },

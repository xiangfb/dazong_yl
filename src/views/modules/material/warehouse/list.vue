...
export default {
  ...
  methods: {
    sortChange (key, dir) {
      this.loading = true
      let sortArr = this.idefinedSort.map(item => {
        return item.key
      })
      if (sortArr.indexOf(key) >= 0) {
        this.$set(this.idefinedSort[sortArr.indexOf(key)], 'dir', dir)
      } else {
    ...
    getMaterialList () {
      ...
      WareHouseService.getList(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        case 'delete':
          ...
          WareHouseService.delete(result).then(res => {

...
import WareHouseService from 'service/modules/material/warehouse/warehouse-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getInfo (id) {
      WareHouseService.detail(id).then(res => {
        if (res.success) {
    ...
    butEvent (type) {
      ...
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/warehouse/list')
          break
      }
    }
  },

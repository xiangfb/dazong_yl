...
import DeptService from 'service/modules/material/dept/dept-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      ...
      DeptService.formData(params).then(res => {
        if (res.success) {
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        DeptService.add(this.param.parentId).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'cadd') {
        DeptService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
      } else if (state === 'delete') {
        DeptService.delete(this.param.id).then(res => {
          if (res.success) {

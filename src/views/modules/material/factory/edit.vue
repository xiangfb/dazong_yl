...
import FactoryService from 'service/modules/material/factory/factory-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getInfo (id) {
      FactoryService.detail(id).then(res => {
        if (res.success) {
    ...
    butEvent (type) {
      ...
      switch (type) {
        case 'back':
          this.$router.replace('/modules/material/factory/list')
          break
      }
    }
  },

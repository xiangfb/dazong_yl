  ...
  methods: {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_LIST') {
        ReportGroupService.trees().then(tree => {
          if (tree.success) {
    ...
    getData () {
      ...
      ReportGroupService.formData(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        case 'delete':
          ...
          ReportGroupService.delete(result).then(res => {
        ...
        case 'default':
          ...
          ReportGroupService.defaulted(frId).then(res => {
            this.getData()
        ...
        case 'upload': // 上传
          ...
          this.action = Config.service.api + '/material/as-report-form/upload/template/' + rId
        ...
        case 'download': // 下载
          ...
          ReportGroupService.downFile(downId).then(res => {
            console.log(res)
    ...
    popupSubmit () { // 按钮点击触发的弹窗需要
      ...
      if (this.flag === 'cfp') {
        ReportGroupService.cancelAllocate({
          entity: {
            archivesIds: venList,
            orgIds: orgList
          }

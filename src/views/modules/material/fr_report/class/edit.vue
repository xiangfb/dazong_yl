...
import ReportClassService from 'service/modules/material/fr_report/report-class-service'
export default {
  name: 'dynamic',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      ...
      ReportClassService.formData(params).then(res => {
        if (res.success) {
    ...
    getMaterialList (one) {
      if (this.formatStyle === 'FORMAT_TREE_FROM') {
        ReportClassService.trees().then(tree => {
          if (tree.success) {
    ...
    submitForm (state, modelData) {
      if (state === 'add') { // 提交成功.
        ...
        ReportClassService.add(this.param.parentId).then(res => {
          if (res.success) {
        ...
        ReportClassService.add(this.param.id).then(res => {
          if (res.success) {
            this.modelData = res.data
          }
        })
        ...
        ReportClassService.delete(this.param.id).then(res => {
          if (res.success) {

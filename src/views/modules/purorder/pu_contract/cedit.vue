...
export default {
  ...
  methods: {
    getMould () { // 获取当前菜单配置
      OptimusPrimeService.optimusPrime({
        ...
        if (res.success) {
          ...
          this.modelData.conId = this.$route.query.contractId
          this.modelData.conCode = this.$route.query.contractCode
    ...
    getInfo (id) {
      PuContractService.detail(id).then(res => {
        if (res.success) {

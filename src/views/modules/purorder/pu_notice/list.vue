...
export default {
  ...
  methods: {
    ...
    getMaterialList () {
      ...
      PuNoticeService.list(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        ...
        case 'copy': // 复制一条信息按钮（字段名称与菜单中配置的名称对应）
          ...
          let id = getList[0].id
          PuNoticeService.copy(id).then(res => {
            this.$message.success(res.msg)
            this.getMaterialList()
          })
          break

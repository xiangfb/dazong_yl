            ...
            <el-col :span="8" class="align-center">
              <el-row>
                ...
                <el-col :span="8">
                  ...
                  <el-input :value="count['dryQuantity'] || specialTableData.dryQuantity" :disabled="true"></el-input>
                </el-col>
                <el-col :span="8">
...
export default {
  ...
  methods: {
    popupConfirm () { // 弹窗资源点击确定触发  data为已选跨页数据
      ...
      FdPuSettlementService.confirmPounds({
        ...
        if (res.success) {
          ...
          // 运费金额
          this.specialTableData.fareMoney = data.freightMoney
          // 原发数
          this.specialTableData.yfSuttle = data.yfSuttle
          // 过磅数量
          this.specialTableData.weighQuantity = data.reSuttle
          // 扣杂
          this.specialTableData.mixed = data.deductWeight
          // 应结金额
          this.specialTableData.shouldSettleMoney = data.calMoney
          // 调整金额
          this.specialTableData.adjustMoney = '0'
          // 运费税率
          this.specialTableData.fareTaxrate = data.fareTaxrate
          // 质量扣重
          this.specialTableData.lossWeight = data.assayDeductQuantity
          // 应结算数量
          this.specialTableData.shouldSettleQuantity = data.calQuantity
          // 实际结算数
          this.specialTableData.realSettleQuantity = data.calQuantity
          // 税率
          this.specialTableData.taxrate = data.taxrate
          // 干基数量
          this.specialTableData.dryQuantity = data.dryBase
          // 备注
          this.specialTableData.remark1 = ''
          // 结算单价
          // this.specialTableData.realSettlePrice = this.specialTableData.shouldSettleMoney / this.specialTableData.realSettleQuantity
          // // 运费单价
          // this.specialTableData.farePrice = this.specialTableData.fareMoney / this.specialTableData.realSettleQuantity
          // // 实结金额
          // this.specialTableData.realSettleMoney = this.specialTableData.adjustMoney + this.specialTableData.shouldSettleMoney
          // // 运费无税单价
          // this.specialTableData.fareNoPrice = this.specialTableData.fareMoney / this.specialTableData.shouldSettleQuantity
          // // 质量扣价
          // this.specialTableData.qualityDeductPrice = this.specialTableData.conPrice - this.specialTableData.realSettlePrice
          // // 无税金额
          // this.specialTableData.realNoMoney = this.specialTableData.realSettleMoney / (1 + 0.01 * this.specialTableData.taxrate)
          // // 无税单价
          // this.specialTableData.realNoPrice = this.specialTableData.realNoMoney / this.specialTableData.shouldSettleQuantity
          // // 运费无税金额
          // this.specialTableData.fareNoMoney = this.specialTableData.fareMoney / (1 + 0.01 * this.specialTableData.fareTaxrate)
        }
      })
    },
    getMould () {
    ...
    butReceipt (e) {
      switch (e) {
        case 'poundList':
          ...
          GlobalService.optimusPrimePopup('330').then(res => {
            if (res.success) {
    ...
    getId () {
      FdPuSettlementService.getId().then(res => {
        if (res.success) {
          this.uuID = res.data
  ...
  computed: {
    count () {
      // 实结金额
      let realSettleMoney = this.retainCount(this.specialTableData.adjustMoney + this.specialTableData.shouldSettleMoney, 2)
      // 无税金额
      let realNoMoney = this.retainCount(realSettleMoney / ((this.specialTableData.taxrate * 0.01) + 1), 2)
      // 运费单价
      let farePrice = this.specialTableData.shouldSettleQuantity === '0' ? 0 : this.retainCount(this.specialTableData.fareMoney / this.specialTableData.realSettleQuantity, 2)
      // 运费无税金额
      let fareNoMoney = this.retainCount(this.specialTableData.fareMoney / (1 + 0.01 * this.specialTableData.fareTaxrate), 2)
      // 无税单价
      let realNoPrice = this.specialTableData.shouldSettleQuantity === '0' ? 0 : this.retainCount(realNoMoney / this.specialTableData.shouldSettleQuantity, 2)
      // 结算单价
      let realSettlePrice = this.specialTableData.shouldSettleQuantity === '0' ? 0 : this.retainCount(this.specialTableData.shouldSettleMoney / this.specialTableData.realSettleQuantity, 2)
      // 运费无税单价
      let fareNoPrice = this.specialTableData.shouldSettleQuantity === '0' ? 0 : this.retainCount(this.specialTableData.fareMoney / this.specialTableData.shouldSettleQuantity, 2)
      // 质量扣价
      let qualityDeductPrice = this.retainCount(this.specialTableData.conPrice - this.specialTableData.realSettlePrice, 2)

      return {
        'realSettleMoney': realSettleMoney,
        'realNoMoney': realNoMoney,
        'farePrice': farePrice,
        'fareNoMoney': fareNoMoney,
        'realNoPrice': realNoPrice,
        'realSettlePrice': realSettlePrice,
        'fareNoPrice': fareNoPrice,
        'qualityDeductPrice': qualityDeductPrice
      }
    }

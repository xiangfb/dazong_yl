...
import MmSaWeighService from 'service/modules/measure/mm_sa_weigh/mm-sa-weigh-service'
...
export default {
  ...
  methods: {
    ...
    getMaterialList () {
      ...
      MmSaWeighService.list(params).then(res => {
        if (res.success) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        ...
        case 'copy':
          ...
          let id = getList[0].id
          MmSaWeighService.copy(id).then(res => {
            this.$message.success('操作成功')
          })
          break
        case 'synchronization':
          MmSaWeighService.syn({
        ...
        case 'down':
          ...
          this.$confirm('是否确认进行冲减操作？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            MmSaWeighService.down(getList[0].id).then(res => {
              if (res.success) {
                this.$message.success(res.msg)
                this.getMaterialList()
              }
        ...
        case 'reversedown':
          ...
          this.$confirm('是否确认进行反冲减操作？', '提示', {
            ...
            MmSaWeighService.reversedown(getList[0].id).then(res => {
              if (res.success) {
                this.$message.success(res.msg)
              }
            })

<template>
  <div class="bg-white">
    <div class="p-10">
      <elx-editable
        v-loading="loading"
        class="confirm_tr_fh_weigh_table"
        ref="elxTable"
        border
        ...
        <el-table-column align="center" min-width="200" fixed="right" label="操作">
          <template v-slot="scope">
            <el-button
              v-for="(type, i) in scope.row.data.confirmSubButs"
              :key="i"
...
import ConfirmTrFhWeighService from 'service/modules/measure/confirm-tr-fh-weigh-service'
export default {
  data () {
    return {
      loading: true,
  ...
  created () {
    this.tableHeader = tableHead.confirmTrFhWeighHead.map(item => {
      if (item.imerge) {
  ...
  methods: {
    ...
    butEvent (type, row) {
      ConfirmTrFhWeighService.submit({
        'entity': {
          'regCodes': [
            row.data.regCode
          ],

<template>
  <div class="bg-white">
    <div class="p-10">
      <elx-editable
        v-loading="loading"
        class="confirm_al_sh_weigh_table"
        ref="elxTable"
        border
...
import ConfirmAlShWeighService from 'service/modules/measure/confirm-al-sh-weigh-service'
export default {
  data () {
    return {
      loading: true,
  ...
  created () {
    this.tableHeader = tableHead.confirmAlShWeighHead.map(item => {
      if (item.imerge) {
  ...
  methods: {
    ...
    butEvent (type, row) {
      ConfirmAlShWeighService.submit({
        'entity': {
          'regCodes': [
            row.data.regCode
          ],

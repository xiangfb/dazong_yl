<template>
  <div class="bg-white">
    <div class="p-10">
      <elx-editable
        v-loading="loading"
        class="confirm_tr_sh_weigh_table"
        ref="elxTable"
        border
...
import ConfirmTrShWeighService from 'service/modules/measure/confirm-tr-sh-weigh-service'
export default {
  data () {
    return {
      loading: true,
  ...
  created () {
    this.tableHeader = tableHead.confirmTrShWeighHead.map(item => {
      if (item.imerge) {
  ...
  methods: {
    ...
    butEvent (type, row) {
      ConfirmTrShWeighService.submit({
        'entity': {
          'regCodes': [
            row.data.regCode
          ],

<template>
  <div class="bg-white">
    <div class="p-10">
      <elx-editable
        v-loading="loading"
        class="confirm_al_fh_weigh_table"
        ref="elxTable"
        border
...
import ConfirmAlFhWeighService from 'service/modules/measure/confirm-al-fh-weigh-service'
export default {
  data () {
    return {
      loading: true,
  ...
  created () {
    this.tableHeader = tableHead.confirmAlFhWeighHead.map(item => {
      if (item.imerge) {
  ...
  methods: {
    ...
    butEvent (type, row) {
      ConfirmAlFhWeighService.submit({
        'entity': {
          'regCodes': [
            row.data.regCode
          ],

...
import ConfirmSaWeighService from 'service/modules/measure/confirm-sa-weigh-service'
export default {
  data () {
    return {
      loading: true,
  ...
  created () {
    this.tableHeader = tableHead.confirmSaWeighHead.map(item => {
      if (item.imerge) {
        ...
        'entitysub': [
          {
            ...
            'deSuttleConfirm': row.data.deSuttleConfirm,
            'deConcentration': row.data.deConcentration
          }
        ]
      }).then(res => {

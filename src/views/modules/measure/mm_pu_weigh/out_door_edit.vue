...
export default {
  ...
  methods: {
    popupConfirm (data, fatherData, index) { // 弹窗资源点击确定触发  data为已选跨页数据
      ...
      PuRegistryService.getSub(data[0].id).then(res => {
    ...
    getInfo (id) {
      MmPuWeighService.detail(id).then(res => {
        if (res.success) {
      ...
      if (state) { // 提交成功.
        ...
        switch (this.$route.query.type) {
          ...
          case 'update':
            ...
            break
          case 'out_door_edit':
            MmPuWeighService.outDoorUpdate({
              entity: modelData,
              entitysub: child
            }).then(res => {
              console.log(res)

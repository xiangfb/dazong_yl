...
export default {
  components: {
    orgTab, tabDetail, assayTab, assayDetail
  },
  props: {
    show: {
      type: Boolean
  ...
  methods: {
    ...
    async addUser () {
      ...
      let result = await roleService.addOrgs({ userId: this.roleId, orgIds }, 'sysUser', 'org')
      if (result) {
        this.userShow = false
      }
    ...
    async addOrder () {
      ...
      let result = await roleService.addOrgs({ userId: this.roleId, assayIds }, 'sysUser', 'ass')
      if (result) {

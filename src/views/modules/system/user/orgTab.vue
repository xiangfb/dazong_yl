      ...
      <basic-container>
        <avue-crud
          ...
          <template slot="menuLeft">
            <el-button @click="addClick" type="primary">添加公司</el-button>
          </template>
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      this.loadingT = true
      menu.orgListC(this.roleId, params, 'sysUser', 'org').then(res => {
        if (res.success) {
    ...
    handleDel (row, index) {
      ...
      this.$confirm('是否删除选中的数据?', '提示', {
        ...
        let obj = { 'userId': this.roleId, orgIds: ids }
        console.log(obj)
        menu.deleteOrgs(obj, 'sysUser', 'org').then(res => {
          if (res.success) {
            this.getData()

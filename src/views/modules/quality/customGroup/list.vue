  ...
  methods: {
    ...
    confirm (selectedsData, fatherData, index, resourceList) {
      if (selectedsData.length === 1) {
        this.cancel()
        this.formatId = selectedsData[0].id// 模板ID赋值
        this.flag = selectedsData[0].flag
        this.getMould()
    ...
    getResAssayFormatId () {
      CustomType.getFormatByCode({ code: 'res_group_format' }, 'format').then(res => {
        if (res.success) {
          if (res.data) {
    ...
    butEvent (e) {
      ...
      switch (e) {
        case 'delete': // 删除按钮（字段名称与菜单中配置的名称对应）
          ...
          break
        case 'refresh': // 刷新按钮（字段名称与菜单中配置的名称对应）
          this.getMaterialList()
          break

...
export default {
  ...
  created () {
    ...
    switch (this.$route.query.type) {
      case 'addFormatSub':
        this.service = 'assayformatSub'
        this.subName = 'qcAssayFormatSubs'
        break
    }
    this.formAllout = tableHead[this.$route.query.type]
  },
  mounted () {
    this.getCustomTablesByCode({ code: this.$route.query.type }, 'format')
  },
  methods: {
    // 获取枚举列表
    getAlert () {
    ...
    getCustomTablesByCode (param, type) { // 获取表数据
      this.isLoading = true
      this.butLoading = true
      CustomType.getCustomTablesByCode(param, type).then(res => {
        if (res.success) {
          let promises = []
          for (let item of res.data) {
            promises.push(this.getFormSubsByFormatAndTable({ cgFormatId: this.$route.query.formatId, cgCustomTableId: item.id }, this.service, item))
          }
    ...
    butFun (type) { // 表格按钮触发
      switch (type) {
        case 'save':
          ...
          AssayType.saveListData({ entity: { [this.subName]: tableData } }, this.service).then(res => {
            if (res.success) {
              this.$notify({
                showClose: true,
                message: '保存成功',
    ...
    getFormSubsByFormatAndTable (param, type, data) {
      return new Promise((resolve, reject) => {
        AssayType.getFormSubsByFormatAndTable(param, type).then(res => {
          if (res.success) {
            this.treeDatas.push({
              data,
              ids: data.name,

  ...
  <div class="card__body_p_10" :style="{'height': `${maxHeight + 5}px`, 'overflow-y': 'auto'}">
    ...
    <basic-container v-show="showAdd" class="p-n">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
           <el-col :span="12">
            <el-form-item prop="fieldEnumId" label="字段枚举">
              <el-select v-model="form.fieldEnumId" filterable  placeholder="请选择">
                <el-option
                  v-for="item in customEnums"
                  :key="item.id"
                  :label="item.enumName"
                  :value="item.id">
                </el-option>
           ...
           <el-col :span="12">
           <el-form-item prop="realValue" label="实际值">
               <el-input v-model="form.realValue"></el-input>
            </el-form-item>
           </el-col>
             <el-col :span="12">
           <el-form-item prop="showValue" label="显示值">
              <el-input v-model="form.showValue"></el-input>
            </el-form-item>
           </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getCustomEnums () {
      // TODO:调用接口  获取枚举表下拉数据
      SourceService.select({}, 'customEnum').then(res => {
        this.customEnums = res.data
      })
    },
    getView (id) {

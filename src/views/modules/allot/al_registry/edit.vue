<template> <!-- 主子表 代表 -->
  <div :style="{height: `${maxHeight}px`}" v-loading="loading">
    <component :is="currentView" :tips="tips" :max-height="maxHeight" ref="componentRef" :fieldData="fieldData" :modelData="modelData" :btnGroup="btnGroup" @submit="submitForm" @reset="resetForm" @but-monitor="butMonitor"></component>
  </div>
</template>
      ...
      if (state) { // 提交成功.
        switch (this.$route.query.type) {
          ...
          case 'update':
            ...
            break
          case 'safeUpdate':
            AlRegistryService.noSafeupdate({
              entity: modelData,
              entitysub: child
            }).then(res => {
              console.log(res)

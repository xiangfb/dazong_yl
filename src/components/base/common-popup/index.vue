<template>
  <div class="resource_box" @click.stop.prevent>
    <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="visibleSync" :show-close="false" :width="formatStyle === 'FORMAT_RESOURCE_TREE' ? '35%' : '75%'">
      <div slot="title">
      ...
      if (val.data && val.data.cgFormatResId) {
        ...
        GlobalService.optimusPrimePopup(val.data.cgFormatResId).then(res => {
          if (res.success) {
            ...
            this.fieldData.detail_id = val.data.id || -1
